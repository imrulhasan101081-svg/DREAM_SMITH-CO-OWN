...
export default async function ProjectDetail({ params }: { params: { slug: string } }) {
  ...
  return (
    <>
      ...
      {/* PROJECT HERO */}
      <section className="bg-navy text-ivory pt-16 sm:pt-20 pb-16 sm:pb-24">
        <div className="max-w-[1180px] mx-auto px-4 sm:px-6 md:px-8">
          <div className="flex items-center gap-3 sm:gap-4 text-gold-bright mb-6 text-[12px] sm:text-[13px] font-mono tracking-wider">
          ...
          <div className="grid md:grid-cols-[1.2fr_0.8fr] gap-10 md:gap-16 items-start mt-6 sm:mt-8">
            <div>
              <span className="font-mono text-[11px] sm:text-[12px] tracking-[0.14em] uppercase text-gold mb-3 sm:mb-4 block">{t('currentlyFunding')}</span>
              <h1 className="font-serif font-normal text-[clamp(32px,4vw,52px)] leading-[1.06] tracking-[-0.01em] mb-4 sm:mb-6">{t('titlePrefix')} <em className="italic text-gold">Chihno</em></h1>
              <p className="text-[15.5px] sm:text-[17px] text-ivory/70 max-w-[580px] leading-relaxed mb-8 sm:mb-10">
                {t('description')}
              </p>
            ...
            <div className="bg-white/5 border border-gold/20 rounded-sm p-5 sm:p-8">
              <h3 className="font-serif text-[20px] sm:text-[22px] text-gold-bright mb-4 sm:mb-6">{t('investmentDetails')}</h3>
              ...
              <div className="flex flex-col">
                {[
                  ...
                  <div key={i} className="flex justify-between items-baseline py-3.5 sm:py-4 border-b border-gold/10">
                    <span className="text-[13px] sm:text-[13.5px] text-ivory/60">{fact.k}</span>
                    <span className="font-serif text-[16px] sm:text-[18px] text-ivory">{fact.v}</span>
                  </div>
                ))}
              </div>

              <div className="mt-6 sm:mt-8">
                <div className="flex justify-between text-[12px] sm:text-[12.5px] mb-2 font-mono text-ivory/60">
                  <span>{t('sharesReserved')}</span><span>176 / 274</span>
                </div>
                <div className="h-1.5 bg-ivory/10 rounded-full overflow-hidden mb-3 sm:mb-4">
                  <div className="h-full w-[64%] bg-gradient-to-r from-gold to-gold-bright rounded-full"></div>
                </div>
                <div className="text-center text-[11.5px] sm:text-[12px] text-gold-bright font-mono mb-6 sm:mb-8">{t('sharesRemaining', { count: 98 })}</div>
              </div>

              <Link href="/apply" className="btn-gold block text-center w-full py-3.5 sm:py-4 text-[14.5px] sm:text-[15px] font-semibold tracking-wide">
                {t('reserveYourShare')}
              </Link>
            </div>
          </div>
      ...
      {/* GALLERY SECTION */}
      <section className="bg-ivory py-16 sm:py-24 border-b border-line">
        <div className="max-w-[1180px] mx-auto px-4 sm:px-6 md:px-8">
          <div className="flex items-end justify-between mb-8 sm:mb-12">
            <div>
              <span className="eyebrow text-[11px] sm:text-[12px] tracking-[0.2em] uppercase text-gold mb-3 sm:mb-4 block font-bold">{t('gallery.eyebrow')}</span>
              <h2 className="font-serif font-normal text-[clamp(26px,3.2vw,40px)] leading-[1.15] tracking-[-0.01em] text-navy">{t('gallery.title')} <em className="italic text-gold">{t('gallery.titleEmphasis')}</em></h2>
            </div>
          </div>
      ...
      <section className="py-16 sm:py-24">
        <div className="max-w-[800px] mx-auto px-4 sm:px-6 md:px-8 text-center">
          <span className="eyebrow text-[11px] sm:text-[12px] tracking-[0.2em] uppercase text-gold mb-3 sm:mb-4 block font-bold">{t('progress.eyebrow')}</span>
          <h2 className="font-serif font-normal text-[clamp(26px,3.2vw,40px)] leading-[1.15] tracking-[-0.01em] mb-3 sm:mb-4">{t('progress.title')} <em className="italic text-gold">{t('progress.titleEmphasis')}</em></h2>
          <p className="text-ink/60 text-[15px] sm:text-[16px] leading-relaxed mb-8 sm:mb-12">{t('progress.description')}</p>

          <div className="text-left bg-white border border-line-light p-5 sm:p-8 rounded-sm shadow-sm relative">
            <div className="sm:absolute top-8 right-8 inline-block mb-3 sm:mb-0 bg-sage/10 text-sage px-3 py-1 text-[11px] font-mono rounded-sm">{t('progress.latest')}</div>
            <div className="font-mono text-[11.5px] sm:text-[12px] text-ink/40 tracking-wider mb-2">12 AUGUST 2026</div>
            <h3 className="font-serif text-[18px] sm:text-[20px] text-navy mb-3 sm:mb-4">Foundation Piling Commenced</h3>
            ...
            <p className="text-[13.5px] sm:text-[14px] text-ink/70 leading-relaxed">
